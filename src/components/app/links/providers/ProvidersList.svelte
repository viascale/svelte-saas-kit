<script lang="ts">
  ...
  import ProvidersListAndTable from "./ProvidersListAndTable.svelte";
  ...
  $: filteredItems = (): LinkDto[] => {
    ...
    return items.filter(
      (f) => f.id?.toUpperCase().includes(searchInput.toUpperCase()) || f.providerWorkspace.name?.toString().toUpperCase().includes(searchInput.toUpperCase())
    );
  };
</script>
