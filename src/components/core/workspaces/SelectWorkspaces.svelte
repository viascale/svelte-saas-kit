<script lang="ts">
  ...
  function toggle(item: WorkspaceDto) {
    if (isSelected(item)) {
  ...
  function accept() {
    const selectedWorkspaces: WorkspaceDto[] = [];
    items.forEach((element) => {
      if (isSelected(element)) {
        selectedWorkspaces.push(element);
      }
    });
    dispatch("selected", { items: selectedWorkspaces });
    close();
  }
    ...
    <div class="fixed z-50 inset-0 overflow-y-auto">
      <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div
          class={classNames(
            maxSize,
            "sm:max-w-lg inline-block align-bottom bg-white rounded-sm px-4 pt-5 pb-4 text-left overflow-visible shadow-xl transform transition-all my-8 sm:align-middle w-full sm:p-6"
          )}
          role="dialog"
          aria-modal="true"
