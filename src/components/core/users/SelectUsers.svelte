<script lang="ts">
  ...
  export function show(_selected: string[]) {
    selected = _selected;
    showing = true;
  ...
  function toggle(item: UserDto) {
    if (isSelected(item)) {
      selected = selected.filter((f) => f !== item.id);
    } else {
      selected = [...selected, item.id];
    }
  }
  $: isSelected = (item: UserDto) => {
  ...
  function accept() {
    ...
    dispatch("selected", { items: selectedUsers });
    close();
  }
  ...
  {#if showing}
    <div class="fixed z-50 inset-0 overflow-y-auto">
      <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        > -->
        <div
          class="sm:max-w-lg inline-block align-bottom bg-white rounded-sm px-4 pt-5 pb-4 text-left overflow-visible shadow-xl transform transition-all my-8 sm:align-middle w-full sm:p-6"
          role="dialog"
          aria-modal="true"
            ...
            <div class="max-w-lg mx-auto">
              <div>
                <div class="text-center">
                  <h2 class="mt-2 text-lg font-medium text-gray-900">{$t("app.workspaces.actions.selectWorkspaceUsers")}</h2>
                  <p class="mt-1 text-sm text-gray-500">{$t("app.workspaces.actions.selectUsers")}</p>
                </div>
                {#if allowSearch}
                        ...
                        <li class="py-2 flex items-center justify-between space-x-3">
                          <div class="min-w-0 flex-1 flex items-center space-x-3">
                            <div class="min-w-0 flex-1">
                              <p class="text-sm font-medium text-gray-900 truncate">{item.email}</p>
